...
public class FirebaseManager : MonoBehaviour
{
    ...
    public async Task AddNotificationToUser(string userID, string message)
    {
        Dictionary<string, object> notificationData = new Dictionary<string, object>
            { { UnityEngine.Random.Range(0,100000000).ToString(), message } }; // Playing a numbers game.
    ...
    //Removes a player's personal listing to a list in their db data, so the market can detect changes in it in real-time, and refresh when necessary.
